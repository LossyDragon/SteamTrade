...
public class FragmentWeb extends FragmentBase {
	...
	public void onResume() {
		...
		if (web_view != null) {
			...
			if (!loaded_page) {
				if (url == null) web_view.loadUrl("https://steamcommunity.com/profiles/" + SteamService.singleton.steamClient.getSteamId().convertToLong());
				else
	...
	private class SteamGuardJavascriptInterface {
		...
		private void setResultOkay(String value) {
			returnStatus = "ok";
			returnValue = value == null ? "" : value;
		}

		private void setResultError(String value, int code) {
			returnStatus = "error";
			returnValue = value == null ? "" : value;
			returnCode = "" + code;
		}

		private void setResultBusy() {
			returnValue = "";
			returnStatus = "busy";
		}
